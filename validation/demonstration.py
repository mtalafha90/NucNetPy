...
def latex_solver_table(rows: List[Dict[str, object]]) -> str:
    lines = [
        ...
        r"\toprule",
        r"Method & Time (s) & $n_{\rm fev}$ & $n_{\rm jev}$ & $\sum_i A_iY_i$ & "
        r"Max.\ rel.\ difference\\",
        r"\midrule",
    ]
    for r in rows:
        ...
        lines.append("%s & %.2f & %d & %d & %.12f & %s\\\\" % (
            r["method"].upper(), r["seconds"], r["nfev"], r["njev"],
            r.get("xsum", float("nan")),
            "reference" if diff == 0.0 else ("%.2e" % diff if diff is not None else "--")))
    lines += [r"\bottomrule", r"\end{tabular}"]
    return "\n".join(lines)
...
def latex_tolerance_table(rows: List[Dict[str, object]]) -> str:
    lines = [
        ...
        r"\toprule",
        r"$r_{\rm tol}$ & $a_{\rm tol}$ & Time (s) & $n_{\rm fev}$ & "
        r"$\sum_i A_iY_i$ & Change vs.\ previous\\",
        r"\midrule",
    ]
    for r in rows:
        if not r.get("success"):
            lines.append(r"%.0e & %.0e & \multicolumn{4}{l}{failed}\\" % (r["rtol"], r["atol"]))
            continue
        change = r.get("max_rel_change")
        lines.append("%.0e & %.0e & %.2f & %d & %.12f & %s\\\\" % (
            r["rtol"], r["atol"], r["seconds"], r["nfev"], r.get("xsum", float("nan")),
            "--" if change is None else "%.2e" % change))
    lines += [r"\bottomrule", r"\end{tabular}"]
    return "\n".join(lines)
